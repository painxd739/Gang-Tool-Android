# https://github.com/TT-Tutorials | https://github.com/TT-Tutorials/RAVAN-Nuker
# Coded / Dev / Owner: ††#1792
# Copyright © RAVAN-Nuker
#########################################################
...
try:
    ...
    with open('data/logins.json', 'w') as f:
            print(f"\n[{g}#\x1b[95m\x1B[37m] Logging Into RAVAN-Nuker")
...
#   JOINER
    if choice == '1':
        ...
        gh = input(f"""
Joiner is in the Paid Version of RAVAN-Nuker!\nIf You Are Wanting to Purchase Make Sure to Checkout the Offical RAVAN-Nuker Website!
                                   
[\x1b[95m1\x1b[95m\x1B[37m] RAVAN-Nuker Website
[\x1b[95m2\x1b[95m\x1B[37m] Exit
  
[\x1b[95m>\x1b[95m\x1B[37m] Choice?: """)

        if gh in ['01','1']:
            webbrowser.open('https://RAVANnuker.org')
        elif gh in ['02','2']:
            exit = spammer()
        else:
         print('')
...
#   ACCOUNT NUKER
    if choice == '7':
        ...
        if threading.active_count() < threads:
            threading.Thread(target=utilities.Plugins.Account_Nuker.RAVANNUKER_START, args=(token, Server_Name, message_Content)).start()
...
#       width = os.get_terminal_size().columns
        ...
        def main():
            ...
            RAVAN = commands.Bot(command_prefix=prefix, intents=discord.Intents().all())
            RAVAN.remove_command('help')

            @RAVAN.event
            async def on_ready():
                if len(RAVAN.guilds) > 1:
                    guildpl = "guilds"
                else:
                    guildpl = "guild"
                activity = discord.Game(name=f"RAVAN-Nuker Server Nuker", type=3)
                await RAVAN.change_presence(status=discord.Status.dnd, activity=activity)
                clear()
                print(f"[\x1b[95m>\x1b[95m\x1B[37m] Bot : {RAVAN.user} ({len(RAVAN.guilds)} {guildpl})")
            ...
            async def nuke(ctx):
                ...
                await ctx.guild.edit(name="RAVAN-Nuker Runs Discord")
...
#   DM SPAMMER
    if choice == '10':
        ...
        gh = input(f"""
DM Spammer is in the Paid Version of RAVAN-Nuker!\nIf You Are Wanting to Purchase Make Sure to Checkout the Offical RAVAN-Nuker Website!
...
#   WEBHOOK SPAMMER
    if choice == '15':
        ...
        def RAVAN():
            ...
            while True:
                for i in range(15):
                    threading.Thread(target=RAVAN).start()
        RAVAN()
...
#   TOKEN GENERATOR
    if choice == '25':
        ...
        gh = input(f"""
Token Generator is in the Paid Version of RAVAN-Nuker!\nIf You Are Wanting to Purchase Make Sure to Checkout the Offical RAVAN-Nuker Website!
...
#   EXIT
    if choice == '32':
        Spinner()
        exit = True if input(f"\n[{Fore.LIGHTMAGENTA_EX}>{Fore.RESET}] Are You Sure You Want To Exit RAVAN-Nuker? Y/N: ").lower() == "y" else spammer() or "n" == sys.exit(0)
    else:
        print(f"")
        time.sleep(0)
        return spammer()
...
if __name__ == "__main__":
                ...
                if os.path.basename(sys.argv[0]).endswith("exe"):
                    search_for_updates()
                    if not os.path.exists(getTempDir()+"\\RAVAN_proxies"):
                       clear()
                else:
                    search_for_updates()
                    if not os.path.exists(getTempDir()+"\\RAVAN_proxies"):
